...
@section('content')
    ...
    <style>
        ...
        /* ════════════════════════════
                                                                    ...
                                                                    ════════════════════════════ */
        .col-foto {
            /* lebar awal dikontrol JS; fallback di sini */
            width: 280px;
            flex-shrink: 0;
            display: flex;
            flex-direction: column;
            ...
            overflow: hidden;
            position: relative;
            /* TIDAK ada border-right di sini — digantikan handle */
        }
        ...
        /* ════════════════════════════
                                                                       RESIZE HANDLE
                                                                       — tab menonjol di tepi kanan kolom foto
                                                                    ════════════════════════════ */
        .resize-handle {
            position: absolute;
            top: 0;
        ...
        /* Label kecil "tarik" di atas grip, muncul saat hover */
        .resize-label {
            position: absolute;
            top: calc(50% - 48px);
            right: 2px;
        ...
        .resize-handle:hover .resize-label {
            opacity: 1;
            color: var(--green-500);
        }

        /* Blokir seleksi teks saat drag */
        body.is-resizing {
            cursor: col-resize !important;
            user-select: none !important;
        }
        ...
        <div class="page-shell">
            ...
            <div class="page-body" id="pageBody">
                ...
                <div class="col-foto" id="colFoto">
                    ...
                    {{-- ══ RESIZE HANDLE ══
                     Tab grip yang menonjol di tepi kanan kolom --}}
                    ...
                    {{-- Desktop: langsung di col-foto | Mobile: di col-foto-bottom --}}
                    <div id="fotoBottom">
                        <label class="foto-upload-label" style="margin-bottom:8px;">
                            <svg viewBox="0 0 24 24">
                    ...
                    <div class="col-fields-scroll">
                        ...
                        <div class="fields-grid" id="fieldsGrid">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-prefix-wrap">
                                    ...
                                    <input type="number" name="harga_beli" min="0" max="100000000"
                                        step="100" value="{{ old('harga_beli', $item->harga_beli ?? 0) }}">
                                </div>
                            </div>
                            ...
                            <div class="form-group field-full">
                                ...
                                <textarea name="deskripsi" placeholder="Keterangan tambahan tentang barang ini...">{{ old('deskripsi', $item->deskripsi ?? '') }}</textarea>
                            </div>

                        </div>{{-- .fields-grid --}}
                    </div>{{-- .col-fields-scroll --}}
    ...
    <script>
        ...
        /* ─────────────────────────────────────────────
           2. Resize — tarik handle ke kanan
           Logika:
           • mousedown/touchstart pada handle → mulai drag
           • mousemove/touchmove              → ubah width col-foto
           • mouseup/touchend                 → selesai, simpan ke localStorage
        ───────────────────────────────────────────── */
        (function() {
            ...
            const MIN_W = 180; // px — lebar minimum kolom foto
            const MAX_W = 560; // px — lebar maksimum
            const WIDE_AT = 340; // px — di atas ini: object-fit contain
            const NARROW_FIELDS = 460; // px — di bawah ini: grid 1 kolom
            ...
            function drag(x) {
                ...
                const minRight = 300; // kolom kanan minimal 300 px

                let w = dragStartW + delta;
                w = Math.max(MIN_W, w);
                w = Math.min(MAX_W, w);
                w = Math.min(w, bodyW - minRight);
            ...
            /* ── Helper: terapkan lebar & class wide ── */
            function setWidth(w) {
                colFoto.style.width = w + 'px';
                colFoto.classList.toggle('wide', w >= WIDE_AT);
            }

            /* ── Helper: responsif grid fields ── */
            function syncGrid() {
                if (!fieldsGrid) return;
                const bodyW = pageBody.getBoundingClientRect().width;
                const fotoW = colFoto.getBoundingClientRect().width;
                const rightW = bodyW - fotoW;
